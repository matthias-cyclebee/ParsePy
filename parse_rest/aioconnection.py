...
class ParseBase(object):
    ...
    @classmethod
    async def execute(cls, uri, http_verb, extra_headers=None, batch=False, _body=None, **kw):
        """
    ...
    @classmethod
    async def DELETE(cls, uri, **kw):
        return await cls.execute(uri, 'DELETE', **kw)

    @classmethod
    async def drop(cls):
        return await cls.POST("%s/schemas/%s" % (API_ROOT, cls.__name__),
...
class ParseBatcher(ParseBase):
    ...
    async def batch_save(self, objects):
        """save a list of objects in one operation"""
        await self.batch(o.save for o in objects)

    async def batch_delete(self, objects):
        """delete a list of objects in one operation"""
        await self.batch(o.delete for o in objects)
