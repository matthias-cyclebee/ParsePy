...
from parse_rest.aioconnection import API_ROOT, ParseBase
from parse_rest.aioquery import QueryManager
...
class Pointer(ParseType):
    ...
    async def from_native(cls, **kw):
        ...
        klass = await Object.factory(kw.get('className'))
...
class EmbeddedObject(ParseType):
    @classmethod
    async def from_native(cls, **kw):
        klass = await Object.factory(kw.pop('className'))
        return klass(**kw)
...
class Relation(ParseType):
    ...
    async def add(self, objs):
        ...
        await self.parentObject.addRelation(self.key,
                                      self.relatedClassName,
                                      objectsId)
    ...
    async def remove(self, objs):
        ...
        for obj in objs:
            if hasattr(obj, 'objectId'):
                objectsId.append(obj.objectId)
        await self.parentObject.removeRelation(self.key,
                                         self.relatedClassName,
                                         objectsId)
...
class File(ParseType, ParseBase):
    ...
    async def save(self, batch=False):
        if self.url is not None:
            raise ParseError("Files can't be overwritten")
        uri = '/'.join([self.__class__.ENDPOINT_ROOT, self.name])
        headers = {'Content-type': self.mimetype}
        response = await self.__class__.POST(uri, extra_headers=headers, batch=batch, _body=self._content)
    ...
    async def delete(self, batch=False):
        uri = "/".join([self.__class__.ENDPOINT_ROOT, self.name])
        response = await self.__class__.DELETE(uri, batch=batch)
...
class Function(ParseBase):
    ...
    async def __call__(self, **kwargs):
        return await self.POST('/' + self.name, **kwargs)
...
class ParseResource(ParseBase):
    ...
    async def __getattr__(self, attr):
        ...
        if not self.__dict__.get('_is_loaded', True):
            del self._is_loaded
            self._init_attrs(await self.GET(self._absolute_url))
    ...
    async def _get_updated_datetime(self):
        return await self.__dict__.get('_updated_at') and self._updated_at._date

    def _set_updated_datetime(self, value):
        self._updated_at = Date(value)

    async def _get_created_datetime(self):
        return await self.__dict__.get('_created_at') and self._created_at._date
    ...
    async def save(self, batch=False):
        if self.objectId:
            return await self._update(batch=batch)
        else:
            return await self._create(batch=batch)

    async def _create(self, batch=False):
        uri = self.__class__.ENDPOINT_ROOT
        response = await self.__class__.POST(uri, batch=batch, **self._to_native())
    ...
    async def _update(self, batch=False):
        response = await self.__class__.PUT(self._absolute_url, batch=batch, **self._to_native())
    ...
    async def delete(self, batch=False):
        response = await self.__class__.DELETE(self._absolute_url, batch=batch)
        if batch:
            return response, lambda response_dict: None
...
class AIOObject(six.with_metaclass(ObjectMetaclass, ParseResource)):
    ...
    def factory(cls, class_name):
        ...
        while types:
            ...
            return type(class_name, (AIOObject,), {})
    ...
    @classmethod
    async def schema(cls):
        """Retrieves the class' schema."""
        root = '/'.join([API_ROOT, 'schemas', cls.__name__])
        schema = await cls.GET(root)
        return schema

    @classmethod
    async def schema_delete_field(cls, key):
        """Deletes a field."""
        root = '/'.join([API_ROOT, 'schemas', cls.__name__])
        payload = {
    ...
    async def increment(self, key, amount=1):
        ...
        await self.__class__.PUT(self._absolute_url, **payload)
    ...
    async def removeRelation(self, key, className, objectsId):
        await self.manageRelation('RemoveRelation', key, className, objectsId)

    async def addRelation(self, key, className, objectsId):
        await self.manageRelation('AddRelation', key, className, objectsId)

    async def manageRelation(self, action, key, className, objectsId):
        objects = [{
                    "__type": "Pointer",
                    "className": className,
                    "objectId": objectId
    ...
    async def addToArray(self, key, objects):
        payload = {
            key: {
    ...
    async def addUniqueToArray(self, key, objects):
        payload = {
            key: {
    ...
    async def removeFromArray(self, key, objects):
        payload = {
            key: {
