...
class QueryManager(object):
    ...
    async def _acount(self, **kw):
        kw.update({"count": 1})
        return await self.model_class.GET(self.model_class.ENDPOINT_ROOT, **kw).get('count')
    ...
    async def fetch(self):
        return await self.all().fetch()

    async def afetch(self):
        return await self.all().afetch()

    async def get(self, **kw):
        return await self.filter(**kw).get()
...
class Queryset(object):
    ...
    async def __anext__(self):
        async for item in self._afetch():
            yield item

    async def _afetch(self, count=False):
        if self._result_cache is not None:
            return len(self._result_cache) if count else self._result_cache
        """
        Return a list of objects matching query, or if count == True return
        ...
        if self._select_related:
            options['include'] = ','.join(self._select_related)
        if count:
            return await self._manager._acount(**options)

        self._result_cache = await self._manager._afetch(**options)
        return self._result_cache
    ...
    async def acount(self):
        return await self._afetch(count=True)

    async def exists(self):
        return await bool(self)

    async def get(self):
        results = await self._afetch()
        if len(results) == 0:
